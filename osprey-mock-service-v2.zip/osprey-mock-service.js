  ...
  return function (req, res) {
    ...
    if (type) {
      ...
      if (Array.isArray(body.examples)) {
        ...
        // if a URL var named __exampleId is found, it is used to find a named Example
        var exampleId = getUrlVars(req.originalUrl).__exampleId
        if( exampleId ) {
          ex = body.examples.find(function (ex) {
            return( ex.name === exampleId )
          })
        }
        if( exampleId && ex ) {
          example = ex.structuredValue ? ex.structuredValue : ex
        } else {
          var newBody = {}
          newBody.examples = body.examples.map(function (ex) {
          ...
          example = getSingleExample(newBody)
        }
      }
